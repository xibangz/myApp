...
<br><h7 class="text font-weight-bold">&ensp;<fmt:message key="products.numb_of_drivers"/></h7></br>
...
<br><h7 class="text font-weight-bold">&ensp;<fmt:message key="products.numb_of_cars"/></h7></br
<div>
...
<car_price:price><p class="text-success font-weight-bold"><fmt:message key="orders.sum"/>:</p><h2>${orderTotal.sum} <fmt:message key="products.price.value"/></h2></car_price:price>
...
<c:if test="${user.passport != null}">
<input type="text" class="text-success text-uppercase font-weight-bold" readonly value="${user.passport}">
</c:if>

<c:if test="${user.passport == null||user==null}">
<a href="registration.jsp" class="btn btn-primary btn-lg active" role="button" aria-pressed="true"><fmt:message key="page.enter_passport"/></a>
</c:if>

<div class="offset-7">

<c:if test="${empty user}"><a href="login.jsp" class="btn btn-success btn-lg active" role="button" aria-pressed="true"><fmt:message key="page.sign_in"/></a>
</c:if>
